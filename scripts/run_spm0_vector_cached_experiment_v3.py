...
EXPERIMENT_COMMIT = "85a006d1b6a2140a7c9590d8c7e6fe04a60a940f"
...
manifest_path=ROOT/"experiments"/"spm0_vector_state_bstar_c_v3.json"
...
train_cache_path=RESULTS/"spm0-vector-v3-train-layer23-cache.pt"
validation_cache_path=RESULTS/"spm0-vector-v3-validation-layer23-cache.pt"
holdout_cache_path=RESULTS/"spm0-vector-v3-holdout-layer23-cache.pt"
...
def train_arm(name,persistent):
    ...
    checkpoint=RESULTS/f"spm0-vector-{name.replace('*','star').lower()}-v3-85a006d.pt"
...
def evaluate_c_reset(case,cache_case):
    ...
    with torch.no_grad():
        ...
        hidden=state_c.reinject(hidden,next_state,last)
        final=tail_forward(hidden,mask)
        final_hidden=final[rows,last]
        logits=torch.nn.functional.linear(final_hidden.float(),choice_weight)
        probabilities=torch.softmax(logits,dim=-1)
...
receipt_path=RESULTS/"SPM0_VECTOR_STATE_BSTAR_C_V3_RUN.json"
