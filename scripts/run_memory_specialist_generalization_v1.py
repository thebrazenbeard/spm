...
EXPERIMENT_COMMIT="71d64795ee517e34deb6ab8f029c6c400e447dd1"
MANIFEST_SHA="2b46e99a35cbbd9b1e44aabceb375b01899b12a01b58dbf9c8fba659902103a0"
...
def converted_reset(case:BenchmarkCase)->BenchmarkCase:
    return BenchmarkCase(
        ...
        risk_class=case.risk_class,tags=case.tags+("stress:reset_base",),
    )

def converted_persistent(case:BenchmarkCase,tokenizer):
    ...
    receipt=memory.retrieve(
        current_text=query,tokenizer=tokenizer,max_retrieved_tokens=512,
    )
    ...
    converted=BenchmarkCase(
        ...
        tags=case.tags+("stress:persistent",),
    )
...
def main():
    ...
    manifest_path=root/"experiments"/"spm_memory_specialist_generalization_v1.json"
    ...
    if sha(data_path)!=spec["data"]["sha256"]:
        raise RuntimeError("stress data digest mismatch")
    ...
    runtime=MemorySpecialistRuntime.load_quantized(
        ...
        base_inventory_digest=BASE_DIGEST,adapter_digest=ADAPTER_DIGEST,
        adapter_config_digest=ADAPTER_CONFIG_DIGEST,microbatch=3,
    )
    source=load_jsonl_cases(data_path)
    resets=tuple(converted_reset(case) for case in source)
    persistent_pairs=tuple(converted_persistent(case,runtime.tokenizer) for case in source)
    persistent=tuple(pair[0] for pair in persistent_pairs)
    receipts=tuple(pair[1] for pair in persistent_pairs)
    ...
    pair=paired(pb,pa)
    gatespec=spec["success_gates"]
    gates={
        "persistent_adapter_correct":
            summaries["PERSISTENT_ADAPTER"]["correct_count"]>=gatespec["persistent_adapter_correct_min"],
        "persistent_adapter_net_gain":
            pair["net_adapter_gain"]>=gatespec["persistent_adapter_net_gain_over_base_min"],
        "full_history_base_correct":
            summaries["FULL_HISTORY_BASE"]["correct_count"]>=gatespec["full_history_base_correct_min"],
        "persistent_adapter_correction_burden":
            summaries["PERSISTENT_ADAPTER"]["correction_burden"]<=gatespec["persistent_adapter_correction_burden_max"],
    }
    receipt={
        "schema_version":1,
        ...
        "retrieval":{
            "all_cases_retrieved":all(bool(r.selected_records) for r in receipts),
    ...
    print(json.dumps({
        ...
        "paired":pair,"retrieval":receipt["retrieval"],
