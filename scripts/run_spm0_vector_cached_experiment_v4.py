...
EXPERIMENT_COMMIT = "49ac955ffcc5102a1f9977767acecf408ed53f05"
...
manifest_path=ROOT/"experiments"/"spm0_vector_state_bstar_c_v4.json"
...
holdout_cases=load_jsonl_cases(ROOT/manifest["data"]["holdout_path"])
...
if len(holdout_cases)!=manifest["data"]["holdout_case_count"]:
    raise RuntimeError("holdout count mismatch")
...
train_cache_path=RESULTS/"spm0-vector-v4-train-layer23-cache.pt"
validation_cache_path=RESULTS/"spm0-vector-v4-validation-layer23-cache.pt"
holdout_cache_path=RESULTS/"spm0-vector-v4-holdout-layer23-cache.pt"
...
holdout_cache=load_or_build_cache(holdout_cases,"eval",holdout_cache_path)
...
if any(tuple(choice.choice_id for choice in case.choices)!=choice_labels for case in (*train_cases,*validation_cases,*holdout_cases)):
...
def cached_training_loss(state,case,cache_case,persistent):
    ...
    hidden=state.reinject_broadcast(hidden,next_state,mask)
...
def train_arm(name,persistent):
    ...
    checkpoint=RESULTS/f"spm0-vector-{name.replace('*','star').lower()}-v4-49ac955.pt"
...
hold_b,hold_c,hold_elapsed=evaluate_dataset(holdout_cases,holdout_cache)
...
def evaluate_c_reset(case,cache_case):
    state_c.eval()
    with torch.no_grad():
        prior=state_c.zero_state(1,device=DEVICE,dtype=torch.float32)
        hidden=cache_case["final_hidden"].to(DEVICE)
        mask=cache_case["final_mask"].to(DEVICE)
        last=mask.long().sum(-1)-1
        rows=torch.arange(hidden.shape[0],device=DEVICE)
        observation=hidden[rows,last].float()
        next_state=state_c.update(
            observation,prior.expand(hidden.shape[0],-1),persistent=True)
        hidden=state_c.reinject_broadcast(hidden,next_state,mask)
        final=tail_forward(hidden,mask)
        final_hidden=final[rows,last]
        logits=torch.nn.functional.linear(final_hidden.float(),choice_weight)
        probabilities=torch.softmax(logits,dim=-1)
...
hold_c_reset=[evaluate_c_reset(case,holdout_cache["cases"][index])
              for index,case in enumerate(holdout_cases)]

def reset_stats(normal,reset):
    normal_only=reset_only=both_correct=both_wrong=0
    for n,r in zip(normal,reset,strict=True):
        if n["correct"] and not r["correct"]: normal_only+=1
        elif r["correct"] and not n["correct"]: reset_only+=1
        elif n["correct"] and r["correct"]: both_correct+=1
        else: both_wrong+=1
    return {
        ...
        "net_normal_advantage":normal_only-reset_only,
    }

val_stats=paired_stats(validation_cases,val_b,val_c)
hold_stats=paired_stats(holdout_cases,hold_b,hold_c)
reset_gate_stats=reset_stats(hold_c,hold_c_reset)
val_b_acc=sum(r["correct"] for r in val_b)
val_c_acc=sum(r["correct"] for r in val_c)
hold_b_acc=sum(r["correct"] for r in hold_b)
hold_c_acc=sum(r["correct"] for r in hold_c)
rules=manifest["decision_rules"]
persistence_gate=(
    val_c_acc>=rules["validation_c_accuracy_min"]
    and val_b_acc<=rules["validation_bstar_accuracy_max"]
    and val_stats["net_paired_wins"]>=rules["validation_net_paired_wins_min"]
    and hold_c_acc>=rules["holdout_c_accuracy_min"]
    and hold_b_acc<=rules["holdout_bstar_accuracy_max"]
    and hold_stats["net_paired_wins"]>=rules["holdout_net_paired_wins_min"]
    and reset_gate_stats["normal_only_correct"]>=rules["holdout_c_reset_normal_only_correct_min"]
    and reset_gate_stats["reset_only_correct"]<=rules["holdout_c_reset_intervention_only_correct_max"]
)
decision={
    "status":"PASS" if persistence_gate else "FAIL",
    "validation":{
        "paired":val_stats,
        "bstar_correct":val_b_acc,
        "c_correct":val_c_acc,
    },
    "holdout":{
        "paired":hold_stats,
        "bstar_correct":hold_b_acc,
        "c_correct":hold_c_acc,
    },
    "holdout_c_reset_ablation":reset_gate_stats,
    "thresholds":rules,
...
receipt={
    ...
    "cache_artifacts":{
        ...
        "holdout":{"sha256":sha256_file(holdout_cache_path),"path":str(holdout_cache_path)},
    },
    ...
    "evaluation":{
        ...
        "holdout":{
            "B*":compact_results(holdout_cases,hold_b),
            "C":compact_results(holdout_cases,hold_c),
            "C_RESET":compact_results(holdout_cases,hold_c_reset),
            "elapsed_seconds":hold_elapsed,
        },
    },
    "decision":decision,
}
receipt_path=RESULTS/"SPM0_VECTOR_STATE_BSTAR_C_V4_RUN.json"
